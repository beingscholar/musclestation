(function ($, app) {
	...
	WpfFrontendPage.prototype.sendFiltersOptionsByAjax = (function ($filtersDataBackend, $queryVars, $filterSettings, $generalSettings, $woocommerceSettings, $shortcodeAttr) {
		...
		var
			productListSelector      = _thisObj.fixSelector($filterSettings['product_list_selector'], 'ul.products'),
		...
		jQuery.sendFormWpf({
			...
			onSuccess: function(res) {
				if(!res.error) {
					...
					if (productListSelector!=='ul.products') {
						var loopContainer = productListElem;
						loopContainer.html(res.data['productHtml']);
					} else {
						var catSelector = wpfGetSelector(res.data['categoryHtml'], true, 'ul.products', 1, false, productContainerElem),
							loopSelector = wpfGetSelector(res.data['loopStartHtml'], true, 'ul.products', 3, false, productContainerElem),
							loopContainer = jQuery(loopSelector, productContainerElem);

						if (!loopContainer.length) {
							// trying to obtain loop container without dynamically change classes with numbers in it names
							loopSelector = wpfGetSelector(res.data['loopStartHtml'], true, 'ul.products', 3, true, productContainerElem);
							loopContainer = jQuery(loopSelector, productContainerElem);
						}
						...
						if (typeof jQuery('.product-categories-wrapper', productContainerElem) !== 'undefined' && jQuery('.product-categories-wrapper > ul.products', productContainerElem).length > 0) {
							...
							jQuery('ul.products', productContainerElem).eq(1).html(res.data['productHtml']);
