...
class WoofiltersWpf extends ModuleWpf {
	...
	public function addCustomTaxQuery( $taxQuery, $data, $mode ) {
		...
		foreach ($data as $key => $param) {
			...
			} else if (strpos($key, 'filter_cat') !== false) {
				if (!empty($param)) {
					$idsAnd = explode(',', $param);
					$idsOr = explode('|', $param);
					$isAnd = count($idsAnd) > count($idsOr);
	 ...
	 * @return array
	 */
	public function getFilterExistsItems( $args, $taxonomies, $calcCategory = null, $prodCatId = false, $querySettings = array()) {
